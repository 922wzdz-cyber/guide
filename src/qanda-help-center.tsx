...
function Mock({ name }) {
  ...
  switch (name) {
    ...
    case "library": return (<MiniScreen label="콴다 컨텐츠 · 자료실"><div className="flex gap-1.5 mb-2 items-center"><span className="text-[9px] font-bold px-2 py-0.5 rounded-full border flex items-center gap-0.5" style={{borderColor:BRAND,color:BRAND}}>콴다 컨텐츠 <ChevronDown className="w-2.5 h-2.5" /></span><span className="text-[9px] px-2 py-0.5 rounded-full bg-slate-100 text-slate-500">공통수학1</span></div><div className="flex gap-3 justify-center py-2">{[["#1E3A5F","기출"],["#1F5F3A","도형"],["#7A1F1F","확통"]].map((c,i)=>(<div key={i} className="flex flex-col items-center"><div className="rounded-sm border border-black/10 flex items-end justify-center pb-2" style={{width:44,height:60,backgroundColor:c[0]}}><div className="w-6 h-2 rounded-sm bg-white/70"/></div><span className="text-[8px] text-slate-500 mt-1">{c[1]}</span></div>))}</div></MiniScreen>);
    ...
    case "changeaccount": return (<MiniScreen label="계정 변경"><div className="flex items-center justify-center gap-2 py-2"><div className="flex flex-col items-center"><div className="w-9 h-9 rounded-full flex items-center justify-center" style={{backgroundColor:PANEL}}><span className="text-base">📱</span></div><span className="text-[7px] text-slate-400 mt-0.5">휴대폰 1개</span></div><span className="text-slate-300">=</span><div className="flex flex-col items-center"><div className="w-9 h-9 rounded-full flex items-center justify-center" style={{backgroundColor:BRAND_SOFT}}><span className="text-base">👤</span></div><span className="text-[7px] text-slate-400 mt-0.5">계정 1개</span></div></div><div className="rounded-lg p-1.5 flex items-center justify-center" style={{backgroundColor:WARN_SOFT}}><span className="text-[8px] font-bold text-center" style={{color:"#B45309"}}>인증 정보 초기화는 고객센터 문의 필요</span></div></MiniScreen>);
...
const steps = [
  ...
  { emoji: "🤝", title: "선생님 매칭", time: "영업일 기준 2일", kw: "선생님 매칭 인생선생님 상담 연락 학습 방향 수업 일정 교재 선정", slides: [
    ...
    { illus: "teacher", title: [{ t: "선생님 " }, { t: "정보를 확인", b: true }, { t: "해요." }], points: [
      ...
      { t: "선생님이 24시간 이내로 연락드려요. 연락이 없으면 고객센터로 문의해 주세요." }] },
  ...
  { emoji: "📚", title: "교재 등록", time: "영업일 기준 2~7일", kw: "교재 등록 책 등록 가능 불가능 검색기 isbn 표지 촬영 승인 심사", slides: [
    { illus: "bookcheck", title: [{ t: "등록 전 " }, { t: "꼭 확인", b: true }, { t: "해 주세요." }],
      ...
      no: [
        ...
        "개념원리, 능률, 이듀, 지학사, 키출판사, 마더텅 교재는 일부 교재만 신청 가능",
      ],
      points: [] },
...
const categories = [
  { id: "learning", title: "수업 활용 & 학습 관리", emoji: "📝", badge: "수업 활용", sub: ["다시보기", "자료실", "숙제", "콴다 프리미엄"],
    ...
    blocks: [
      ...
      { heading: "자료실은 어디서 확인하나요?", illus: "library_book", illus2: "library", kw: "자료실 교재 콴다 컨텐츠 자료 기출 도형 확통",
        points: [
        ...
        { t: "[콴다 컨텐츠]에서는 콴다과외가 준비한 자료를 볼 수 있어요." },
  ...
  { id: "trouble", title: "문제 해결", emoji: "⚠️", badge: "문제 해결", sub: ["선생님 미참여", "잦은 일정 변경", "기타 문의"],
    ...
    blocks: [
      { heading: "수업이 예정대로 진행되지 않았어요.", illus: "trouble", kw: "노쇼 미참여 선생님 안 들어옴 일정 지연 보강 잦은 변경 문제",
        ...
        { t: "반드시 고객센터(전화 또는 채팅) 로 연락이 필요해요. 앱에서 자동으로 처리되지 않아요.", warn: true }] },
